...
**Addendum:** `invoiceDate` is sourced from a new field, `invoice_date`, added to `CreateBillDto` (the direct API payload only — `apps/api/src/bills/dto/create-bill.dto.ts`). It is **optional** and **additive**: existing callers that omit it are unaffected, and the field is never inferred from `sale_at` — `sale_at` is transaction/settlement time, `invoice_date` is the document date, and the two are deliberately kept distinct rather than conflated (the same discipline this decision already applies to not fabricating a date for pre-existing bills). Treated with the same explicit-contract-naming weight D-27/F-7 gave a response-contract change, though the risk class is lower: an optional new request field cannot break an existing caller, per D-20's precedent that widening a payload with an optional field is one-way-compatible.

**Enforcement point, named precisely:** the real D-28 key-set enforcement for this field is a **new** test asserting `Object.keys()` of the literal `snapshot` object `bills.service.ts` (P-2) actually writes for a `TAX_INVOICE` bill — exact match, fails on any unexpected key. Two pre-existing tests are **not** this enforcement and must not be mistaken for it: `links.service.spec.ts`'s `ALLOWED_BILL_KEYS`/`ALLOWED_ITEM_KEYS` test checks `LinksService.resolve()`'s Prisma-select shape against a hand-mocked row, where `snapshot` is one opaque key regardless of its contents; `preview-fixtures.spec.ts`'s `SNAPSHOT_WHITELIST` checks synthetic preview fixtures are a *subset* of an allowed set, not an exact match against a real writer's output. Neither previously enforced what P-2 actually writes — this task adds the first test that does.
...
### D-95 · Bill.layoutSnapshot immutability is self-certified per row (layoutSnapshotHash), not verified against a committed baseline file

**Decision:** `Bill` gains a nullable column, `layoutSnapshotHash String?`, written once at creation (both `callbacks.service.ts`'s P-1 path and `bills.service.ts`'s P-2 path) as `hashSnapshot(layoutSnapshot)` — the same SHA-256-over-canonical-JSON function `pnpm verify` already used, now extracted into a shared `src/common/` module and imported by both the write path and the verify script, never duplicated. `pnpm verify`'s immutability check is rewritten to iterate every row in `Bill` and compare each one's current `hashSnapshot(layoutSnapshot)` against its own stored `layoutSnapshotHash` — no baseline-membership branch, no distinction between a fixture-seeded bill and a live-created one. A one-time backfill script (`backfill-layout-snapshot-hash.ts`, matching the existing `backfill-layout-snapshot.ts` pattern) populates `layoutSnapshotHash` for every pre-existing bill from its current, trusted-as-of-today `layoutSnapshot` value. Once backfill confirms every row self-certifies, the committed `prisma/fixtures/layout-snapshot-baseline.json` file and its `--write-baseline` flag are retired — deleted, not kept running alongside the new mechanism.

**Reason the baseline file couldn't be extended, not just narrowed:** the file's check only ever iterated the ids already present as keys inside it — a bill created after the last `--write-baseline` commit was never a key, so it was never inspected, not merely inspected-and-passed. Verifying immutability requires comparing a row's current content against its value at the moment of creation; the baseline file could only ever capture that moment for a bill an operator remembered to re-baseline and commit, which structurally excludes every bill created through the live API or a forgotten fixture update. A tempting-looking fix — auto-adopt a new bill's current hash into the baseline the first time verify sees it — is worse than doing nothing: if the row was already tampered with before that first run, the corrupted value becomes the permanently trusted baseline, laundering the exact violation the check exists to catch. No file-based mechanism can know a value "at issue" for a row it never witnessed at issue time; only the write path itself can.

**Reason a DB column can:** the moment of creation is the one place the trusted value genuinely exists. Writing `layoutSnapshotHash` in the same transaction as `layoutSnapshot` itself (both P-1 and P-2 already write `layoutSnapshot` as a frozen, never-updated field per `TEMPLATE_SYSTEM_v2` §7) makes every bill — fixture-seeded or customer-issued — carry its own tamper-evidence with no dependency on a human remembering to run a script and commit its output. This generalizes the guarantee from "the fixture bills we remembered to track" to "every bill in the database," which is the literal roadmap requirement.

**Migration shape:** one additive, nullable column (`layoutSnapshotHash String?`), mirroring `layoutSnapshot Json?`'s own nullability for the same reason — a bill with no snapshot has nothing to hash, and pre-existing rows have neither value until backfilled.

**Backfill:** treats current DB content as the trusted floor, identical in spirit to what `--write-baseline` already did manually — the only difference is this floor is set once, per row, permanently, rather than re-declared by hand on every fixture change.

**Retirement, deliberately, not a parallel mechanism:** keeping the JSON baseline file running alongside a DB column that provides a strictly stronger, unconditional version of the same guarantee would be two independent implementations of one invariant — the exact pattern this project has avoided everywhere else it came up (D-30's single shared block manifest; Q-1's single narrow P2002 predicate; Q-2's single masking implementation). The file is deleted once backfill confirms every row carries a valid `layoutSnapshotHash`.

### D-96 · The layoutSnapshot.blocks double-wrap regression (T-5 aftermath): root cause, fix, and disposal of the two affected bills

**Decision:** `bills.service.ts` (P-2) and `callbacks.service.ts` (P-1) both wrote `layoutSnapshot.blocks: template.layoutSchema` directly. This was correct when written (commit `593ece4`) — at the time, `Template.layoutSchema` was a plain v1 block array. T-5's migration (`e100a9f`) later converted every `Template.layoutSchema` row to the v2 envelope shape (`{ schemaVersion: 2, skeleton, blocks: [...] }`) but never updated these two write sites, so `Bill.layoutSnapshot.blocks` silently became the whole v2 envelope object instead of the plain array it declares. Both sites now extract `.blocks` explicitly via the existing `LayoutSchemaV2` type (`@digital-billing/block-manifest`, already imported elsewhere in this codebase): `(template.layoutSchema as LayoutSchemaV2).blocks`.

**Why it went undetected:** the only consumer of `layoutSnapshot.blocks` is the public bill-view page (`apps/web/app/(main)/[identifier]/page.tsx`), whose `isBillViewPayload` guard requires `Array.isArray(layoutSnapshot.blocks)` — a double-wrapped bill fails this check and renders a generic `ErrorState`, with nothing logged server-side to distinguish it from a network blip. Confirmed by grep that no other consumer (`links.service.ts` passes `layoutSnapshot` through untouched; `portal-bills.service.ts` excludes it entirely per D-48) needed accounting for. `bills.service.spec.ts`/`callbacks.service.spec.ts`'s unit tests mock `template.layoutSchema` as a plain array — the pre-T-5 shape — so nothing in the suite ever exercised a real v2-shaped template through this exact assembly path.

**Blast radius, checked directly against the database, not assumed:** every `Bill` row was scanned and classified by its actual `layoutSnapshot.blocks` shape. 60 of 62 rows were correctly array-shaped; the only 2 double-wrapped rows were bills created minutes earlier in the same session, through the demo endpoints, specifically to verify Q-5 live. No bill created through ordinary use — before or after T-5 — exhibited the bug in this database; the regression window between T-5 shipping and this fix apparently saw zero real bill creation until these two verification calls.

**Disposal of the two affected bills: deleted, not repaired.** Both were confirmed disposable test artifacts (created by the assistant, in this session, for verification purposes) — not real business records anyone had viewed, shared, or relied on. Deletion was chosen deliberately over repairing `layoutSnapshot.blocks` in place and recomputing `layoutSnapshotHash` to match: D-95's hash is designed to be written once, at creation, and never touched again — there is no sanctioned repair-and-recertify path, and building one to fix two pieces of test data would establish exactly the precedent D-95 exists to foreclose (a legitimate-looking write path for changing already-certified content after the fact). Deleting the rows entirely means D-95's guarantee was never asked to make an exception.

### D-97 · TAX_SUMMARY's forbidden per-rate matrix ('legacy_matrix') removed entirely — 'aggregate' is now unconditional

**Decision:** Q-6 removes the `legacy_matrix` TAX_SUMMARY variant completely — the type, the computation branch, the JSX rendering, the now-orphaned CSS (`.bill-card--minimalist`/`.bill-card--thermal`'s `.bill-tax-summary-row`/`--head` rules), and the `TaxSummaryRow` interface. `renderBlock()`'s `TAX_SUMMARY` case no longer branches on `block.props.mode` at all — every `TAX_SUMMARY` block, from any template, now unconditionally computes and renders the §5 aggregate ladder (one CGST row, one SGST row, or one IGST row inter-state, each summed across all tax rates). `props.mode` becomes vestigial on existing seed data (harmless to leave, since nothing reads it anymore).

**Reason this goes further than `TAX_COMPLIANT`'s own fix:** V-5 already pointed `TAX_COMPLIANT`'s seeded template at `mode: 'auto'`, so no live bill or template has exercised `legacy_matrix` since. But the branch remained reachable in principle by any future custom template a merchant builds via the builder UI with a `TAX_SUMMARY` block and no explicit `mode` prop — the palette has no reason to set it, and a merchant has no way to know the bare default silently reintroduces the exact matrix `TEMPLATE_SYSTEM_v2` §5 forbids. Making `aggregate` unconditional closes that gap structurally rather than relying on every future template author remembering to opt in, and is what makes "the forbidden matrix appears nowhere in the codebase" true by construction rather than true only by current seed-data coincidence.

**Confirmed presentation-only, per D-87's boundary:** no bill or template in the current database has ever exercised `legacy_matrix` (verified directly — every seeded template with a `TAX_SUMMARY` block already sets `mode: 'auto'`; the two RECEIPT skeletons have no `TAX_SUMMARY` block at all; UTILITY has no money blocks). The aggregation arithmetic itself is unchanged — same `snapshot.items` fields read, same `BigInt` summation — only which output shape a `TAX_SUMMARY` block can ever produce changes, and no existing bill's rendering is affected since none was ever eligible for the removed branch.
